...
* Last completed WP: **WP-023 — Partial exam completion and per-question failure reporting**
* Current/next planned WP: awaiting architect review
* Overall phase: **Fresh implementation from approved project architecture**
* Repository implementation state: **WP-023 complete; V1 ACCEPTED WITH KNOWN LIMITATIONS (WP-018), hardened through WP-019–WP-023**
...
* Total: **940** (839 through WP-018, plus WP-019–WP-022 growth, plus WP-023's own net +23 across unit/integration - question-local-vs-system-level orchestration scenarios, renumbering-with-a-gap, `ExamAudit`/`FailedQuestionAudit` model invariants, partial/all-failed audit and output-bundle construction, CLI `PARTIAL`/all-failed reporting, and one full-pipeline `PARTIAL` integration test)
* Passing: **940**
* Failing: **0**
...
WP-001 through WP-023 are complete. **V1 ACCEPTED WITH KNOWN LIMITATIONS** (WP-018), then hardened through five further hardening/reliability WPs (WP-019–WP-023). Every layer through `src/exam_generator/cli` exists, has been confirmed working end-to-end, and a full-plan orchestration run now normally reaches the end of its plan - completing fully, or completing partially with every failure honestly reported - rather than aborting outright on the first isolated question-local failure. The single most important things for the next WP's author to know:

* **WP-023's question-local/system-level split is deliberately minimal** - only `QuestionAttemptsExhaustedError`, `InvalidGroundingOutputError`/`InvalidTextbookOutputError` (after WP-021 exhausts), and duplicate-replacement exhaustion are question-local (recorded, orchestration continues). Everything else that was already "operational" per WP-019 (auth/rate-limit/connection/config, `RetrievalError`, `PromptError`, `GenerationError`, a raw `pydantic.ValidationError`, an exhausted `LLMStructuredOutputError`) remains system-level and still aborts the whole run immediately. Do not casually reclassify more failure types as question-local without checking whether an existing test already asserts system-level treatment for it - two such pre-existing tests directly contradicted an earlier, broader draft classification during WP-023's own implementation.
* **WP-022's own residual finding is explicitly deferred, not yet fixed**: a targeted 5-run diagnostic (`evaluation/wp022_targeted_diagnostic_summary.md`) found the `תאי מערכת העצבים`/INDEPENDENT category/mode's exhaustion is driven by a highly reproducible generation-side hallucinated chunk ID (`STUDENT_SUMMARY:student_summary_1.pdf:0002:0002`, 9/9 recurrences), not a validator-side issue. A plausible future WP-024 would investigate generation-side local evidence references analogous to WP-022's validator-side fix - WP-023 explicitly did not touch this, since with WP-023 in place this specific failure now surfaces as one clearly-reported `PARTIAL` failure rather than aborting the whole exam, lowering the urgency without eliminating the root cause.
* **`ExamGenerationResult`/`ExamOutputBundle.exam` are now `ExamOutput | None`** - `None` only in the all-planned-questions-failed edge case. Any caller pattern-matching on `.exam` being always-present needs updating; the CLI and `output/audit.py` both already handle `None` correctly and can be used as the reference pattern.
* **A residual, much-lower-rate provenance-invention failure mode remains a known, honestly-documented limitation** (unchanged since WP-018/WP-021/WP-022) - the existing strict provenance checks correctly fail closed on it, and it is now WP-023's motivating case for graceful partial completion rather than a blocker.
* **TF-IDF retrieval remains evidenced as sufficient for V1** - Recall@3=86.5%, Recall@5/@8=94.6%, unchanged since WP-017; no WP since has touched retrieval.
...
* **No exam-level semantic diversity, historical-reference rotation, or answer-position balancing exists** - unchanged, still out of scope, still deferred.
...
**V1 is accepted (with known limitations); WP-023 (partial exam completion) is also complete.** No WP-024 has been supplied yet; the next implementation task awaits the architect/user's review of this WP-023 completion report before any further work (including the WP-022-deferred generation-provenance question noted above, or anything else) is defined. Claude must not invent or begin a WP-024 specification.
