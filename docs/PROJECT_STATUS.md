...
* Last completed WP: **WP-015 — Exam output and audit trail**
* Current/next planned WP: **WP-016**
* Overall phase: **Fresh implementation from approved project architecture**
* Repository implementation state: **WP-015 complete**
...
No embeddings, vector databases, semantic/neural retrieval, or CLI functionality has been implemented yet. All five candidate-quality validators (grounding, MCQ, category, quality, textbook) exist as independent single-purpose checks; WP-013 combines them into a working single-question production cycle; WP-014 combines multiple production cycles into a complete, deterministically-planned, duplicate-protected exam; WP-015 turns a successful exam-generation result into the two final, separated output artifacts (clean exam JSON + audit JSON), ready for file writing.
...
* `.venv/bin/python -m pytest -v` → 743 passed, in ~4s with zero network access and no `OPENAI_API_KEY` set in the shell used for the offline run - satisfies the WP's mandatory offline-test requirement.
...
* Real-data smoke test (WP-015): one real `ExamOrchestrator.generate_exam()` call piped directly through `build_exam_output_bundle()`/serialization - see "Real-Data Smoke Test (WP-015)" above. WP-015 itself made zero LLM calls, live or mocked.
...
WP-001 through WP-015 are complete. `src/exam_generator/config` provides configuration loading; `src/exam_generator/models` provides every domain contract including `GeneratedQuestionResponse` and (new in WP-015) `QuestionAttemptAudit`; `src/exam_generator/historical`, `src/exam_generator/ingestion`, `src/exam_generator/chunking`, `src/exam_generator/retrieval` provide historical data, PDF extraction, chunking/corpora, and local retrieval respectively; `src/exam_generator/llm` provides the provider-independent LLM abstraction + OpenAI provider; `src/exam_generator/prompts` provides the external prompt repository/rendering/formatting infrastructure and all seven production prompt files; `src/exam_generator/generation` provides `QuestionGenerator` (category -> `CandidateQuestion`); `src/exam_generator/validation` provides `GroundingValidator`, `MCQValidator`, `CategoryValidator`, `QualityValidator`, and `TextbookValidator`; `src/exam_generator/production` provides `QuestionProducer` (single-question, deterministic-acceptance, bounded-regeneration production cycle); `src/exam_generator/orchestration` provides `ExamOrchestrator` (`ExamRequest` -> complete `ExamOutput` + full plan/production history); `src/exam_generator/output` now provides `build_exam_audit()`/`build_exam_output_bundle()`/serialization+file-writing helpers, turning a successful `ExamGenerationResult` into the two final, separated artifacts (clean exam JSON, audit JSON) with zero LLM calls. The full pipeline through exam-output construction has been confirmed working end-to-end against real data. None of the following exists yet:

* **No CLI exists yet.** Nothing yet reads an `ExamRequest` from a file/argument, invokes `ExamOrchestrator`, calls WP-015's output layer, and writes `exam_<timestamp>.json`/`.audit.json` to a real, timestamped location - `write_exam_json()`/`write_audit_json()` (WP-015) take a fully caller-supplied path and apply no naming/timestamp policy themselves, by design (that policy is explicitly WP-016's job).
* **No exam-level semantic diversity, historical-reference rotation, or answer-position balancing exists.** Unchanged from the WP-014 note - still out of scope, still deferred.
* **No global operational-failure retry exists at any layer.** Unchanged from the WP-014 note. WP-015 makes zero LLM calls itself, so it did not add to this; the live reliability characteristics documented under WP-014's "Known Issues" remain the relevant data point for future work in this area.
* **Two audit-provenance gaps remain open, by deliberate WP-015 scope decision, not oversight**: `QuestionAudit.historical_reference_id` and `.evidence` are always `None`/`[]` (genuinely unavailable without threading new return values through already-shipped `QuestionGenerator`/`GroundingValidator`/`TextbookValidator` - judged out of scope as "broad refactoring of previous WPs," per WP-015's own explicit instruction not to do that). Grounding-evidence *identity* specifically is NOT missing - it's preserved intact inside each attempt's own `grounding.evidence_chunk_ids`/`.evidence_text`. Prompt-version/hash provenance is also not captured anywhere (no existing contract field requires it). A future WP could close either gap with a "smallest architectural addition" at the generator/validator layer if richer audit provenance is ever prioritized - not attempted here.
* **Evidence/quote fidelity remains a well-established, non-zero background failure rate** - unchanged from the WP-014 note; WP-015's own live smoke test happened to hit zero such failures, but that is one data point, not evidence the issue is resolved.

The environment's Python interpreter is 3.12.3, not 3.14 as WP-001.md assumed; see Known Issues above. `openpyxl>=3.1` (3.1.5), `pymupdf>=1.24` (1.28.0), `scikit-learn>=1.4` (1.9.0), and `openai>=2.0` (2.52.0) are the real runtime dependencies so far; WP-008 through WP-015 all added none. The next WP's author should account for the environment note when specifying dependencies/tooling.
...
**WP-016** (per the roadmap: CLI). Claude must not invent or begin WP-016's specification; wait for it to be supplied.
