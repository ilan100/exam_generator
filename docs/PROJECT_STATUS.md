...
* Last completed WP: **WP-014 — Exam orchestration**
* Current/next planned WP: **WP-015**
* Overall phase: **Fresh implementation from approved project architecture**
* Repository implementation state: **WP-014 complete**
...
No embeddings, vector databases, semantic/neural retrieval, output-file writing, final audit serialization, or CLI functionality has been implemented yet. All five candidate-quality validators (grounding, MCQ, category, quality, textbook) exist as independent single-purpose checks; WP-013 combines them into a working single-question production cycle; WP-014 combines multiple production cycles into a complete, deterministically-planned, duplicate-protected exam.
...
* Total: **701** (10 from WP-001 + 97 from WP-002 + 59 from WP-003 + 47 from WP-004 + 80 from WP-005 + 71 from WP-006 + 1 category-mapping config test + 49 from WP-007 + 110 from WP-008 + 28 from WP-009 + 22 from WP-010 + 43 from WP-011 + 25 from WP-012 + 30 from WP-013 + 1 new max_duplicate_replacement_attempts config test + 28 from WP-014)
* Passing: **701**
* Failing: **0**
...
* `.venv/bin/python -m pytest -v` → 701 passed, in ~4s with zero network access and no `OPENAI_API_KEY` set in the shell used for the offline run - satisfies the WP's mandatory offline-test requirement.
...
WP-001 through WP-014 are complete. `src/exam_generator/config` provides configuration loading; `src/exam_generator/models` provides every domain contract including `GeneratedQuestionResponse`; `src/exam_generator/historical`, `src/exam_generator/ingestion`, `src/exam_generator/chunking`, `src/exam_generator/retrieval` provide historical data, PDF extraction, chunking/corpora, and local retrieval respectively; `src/exam_generator/llm` provides the provider-independent LLM abstraction + OpenAI provider; `src/exam_generator/prompts` provides the external prompt repository/rendering/formatting infrastructure and all seven production prompt files; `src/exam_generator/generation` provides `QuestionGenerator` (category -> `CandidateQuestion`); `src/exam_generator/validation` provides `GroundingValidator`, `MCQValidator`, `CategoryValidator`, `QualityValidator`, and `TextbookValidator`; `src/exam_generator/production` provides `QuestionProducer` (single-question, deterministic-acceptance, bounded-regeneration production cycle); `src/exam_generator/orchestration` now provides `ExamOrchestrator` (`ExamRequest` -> complete `ExamOutput` + full plan/production history, or a clear per-question failure), combining all seven of the above into a working multi-question exam-generation pipeline. The full exam-orchestration path has been confirmed working end-to-end against the real OpenAI API. None of the following exists yet:

* **No output-file writing, final audit serialization, or CLI exists yet.** `ExamGenerationResult` (WP-014) carries everything `QuestionAudit`/`ExamAudit` (`src/exam_generator/models/audit.py`) would need (every accepted candidate's full validation/attempt/duplicate-replacement history), but nothing yet converts one into the other, or writes `exam_<timestamp>.json`/`.audit.json`. This is WP-015's job.
* **No exam-level semantic diversity, historical-reference rotation, or answer-position balancing exists.** WP-014 implements only exact-normalized-duplicate rejection (per its own explicit scope); embedding/semantic similarity, LLM-based diversity judgment, and forced historical-reference/answer-position rebalancing all remain explicitly out of scope, deferred to a later, focused WP if ever pursued.
* **No global operational-failure retry exists at any layer.** An `LLMProviderError`/invalid-structured-response/retrieval-infrastructure failure at any point (generation, any of the five validators) propagates immediately through `QuestionProducer` and `ExamOrchestrator` alike and aborts the entire exam - by design (WP-013/WP-014 both explicitly exclude this), but live testing this WP showed this is not a purely theoretical case: a handful of real operational failures occurred across roughly a dozen live calls made while investigating and smoke-testing WP-014 (see "Known Issues" for the full breakdown). Whether a bounded, clearly-scoped operational retry belongs at some future layer is an open question this WP deliberately did not resolve.
* **`QuestionAudit`/`ExamAudit` still have no assembly logic** - unchanged from the WP-013 note; `ExamGenerationResult.productions` (WP-014) now carries the full multi-question history that assembly would need.
* **Evidence/quote fidelity is now a well-established three-time pattern.** WP-010 fixed `evidence_chunk_ids` prefix-dropping; WP-013 fixed `TextbookCheckResult.reference_text` paraphrasing; WP-014 fixed an empty-string-vs-null ambiguity in that same WP-013 fix's own wording. Live testing during WP-014 also showed that *even after* the two textbook-prompt fixes, occasional non-verbatim citations and (once) an invented `evidence_chunk_ids` value still occurred - clearer wording measurably narrows this failure mode but does not eliminate it. Any future WP relying on high live success rates from these specific prompts should budget for this as a known, non-zero background failure rate, not assume it is now fully solved.

The environment's Python interpreter is 3.12.3, not 3.14 as WP-001.md assumed; see Known Issues above. `openpyxl>=3.1` (3.1.5), `pymupdf>=1.24` (1.28.0), `scikit-learn>=1.4` (1.9.0), and `openai>=2.0` (2.52.0) are the real runtime dependencies so far; WP-008 through WP-014 all added none. The next WP's author should account for the environment note when specifying dependencies/tooling.
...
**WP-015** (per the roadmap: exam + audit output). Claude must not invent or begin WP-015's specification; wait for it to be supplied.
