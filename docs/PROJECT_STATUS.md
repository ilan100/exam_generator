...
* Last completed WP: **WP-016 — CLI and application entry point**
* Current/next planned WP: **WP-017**
* Overall phase: **Fresh implementation from approved project architecture**
* Repository implementation state: **WP-016 complete**
...
No embeddings, vector databases, or semantic/neural retrieval have been implemented. All five candidate-quality validators (grounding, MCQ, category, quality, textbook) exist as independent single-purpose checks; WP-013 combines them into a working single-question production cycle; WP-014 combines multiple production cycles into a complete, deterministically-planned, duplicate-protected exam; WP-015 turns a successful exam-generation result into the two final, separated output artifacts; WP-016 exposes the complete pipeline as a real, installable command-line tool. The V1 architecture described in the master brief is now fully implemented end-to-end.
...
* `.venv/bin/python -m pytest -v` → 770 passed, in ~4s with zero network access and no `OPENAI_API_KEY` set in the shell used for the offline run - satisfies the WP's mandatory offline-test requirement.
...
WP-001 through WP-016 are complete. `src/exam_generator/config` provides configuration loading; `src/exam_generator/models` provides every domain contract including `GeneratedQuestionResponse` and `QuestionAttemptAudit`; `src/exam_generator/historical`, `src/exam_generator/ingestion`, `src/exam_generator/chunking`, `src/exam_generator/retrieval` provide historical data, PDF extraction, chunking/corpora, and local retrieval respectively; `src/exam_generator/llm` provides the provider-independent LLM abstraction + OpenAI provider; `src/exam_generator/prompts` provides the external prompt repository/rendering/formatting infrastructure and all seven production prompt files; `src/exam_generator/generation` provides `QuestionGenerator` (category -> `CandidateQuestion`); `src/exam_generator/validation` provides `GroundingValidator`, `MCQValidator`, `CategoryValidator`, `QualityValidator`, and `TextbookValidator`; `src/exam_generator/production` provides `QuestionProducer` (single-question, deterministic-acceptance, bounded-regeneration production cycle); `src/exam_generator/orchestration` provides `ExamOrchestrator` (`ExamRequest` -> complete `ExamOutput` + full plan/production history); `src/exam_generator/output` provides `build_exam_audit()`/`build_exam_output_bundle()`/serialization helpers; `src/exam_generator/cli` now provides the installed `exam-generator generate` command, a thin adapter tying the entire pipeline together into a real, runnable tool. The complete pipeline - request file to exam.json/audit.json on disk - has been confirmed working end-to-end via the actual CLI entry point against the real OpenAI API. The V1 architecture described in the master brief is now fully implemented. None of the following exists yet:

* **No integration/end-to-end test suite beyond the unit-level mocked CLI tests and the individual WPs' own live smoke tests exists yet.** Each WP has its own offline unit tests plus one manual live smoke test; nothing yet runs a broader, dedicated integration suite spanning multiple real categories/request shapes/failure modes in one coordinated test run. This is WP-017's job per the roadmap.
...
* **No global operational-failure retry exists at any layer**, and this WP's own CLI smoke test reinforces why that might eventually matter: earlier WPs' live testing already showed a non-zero background operational-failure rate (see WP-014's "Known Issues"), and WP-016's CLI surfaces that same risk directly to an end user as a plain exit-code-1 failure with no automatic retry, by design. Whether a bounded, clearly-scoped retry belongs at some future layer remains an open question no WP has resolved yet.
* **Two audit-provenance gaps remain open, by deliberate WP-015 scope decision, not oversight** - unchanged from the WP-015 note: `QuestionAudit.historical_reference_id`/`.evidence` are always `None`/`[]`; prompt-version/hash provenance is not captured anywhere. Neither was revisited during WP-016, which only consumes the existing output layer, never modifies it.
* **Evidence/quote fidelity remains a well-established, non-zero background failure rate** - unchanged from the WP-014 note. WP-016's own single live smoke test happened to hit zero such failures (and needed only 1 quality-rejected regeneration, not an operational failure), but that is one more data point, not evidence the underlying issue is resolved.

The environment's Python interpreter is 3.12.3, not 3.14 as WP-001.md assumed; see Known Issues above. `openpyxl>=3.1` (3.1.5), `pymupdf>=1.24` (1.28.0), `scikit-learn>=1.4` (1.9.0), and `openai>=2.0` (2.52.0) are the real runtime dependencies so far; WP-008 through WP-016 all added none (WP-016 added a console-script entry point, not a new dependency). The next WP's author should account for the environment note when specifying dependencies/tooling.
...
**WP-017** (per the roadmap: integration tests). Claude must not invent or begin WP-017's specification; wait for it to be supplied.
